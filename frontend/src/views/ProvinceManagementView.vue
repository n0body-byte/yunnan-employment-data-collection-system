<template>
  <DashboardShell
    ...
    title="省级综合管理"
    subtitle="统一处理备案审核、终审归档、用户角色、系统维护和数据交换。"
    :nav-items="navItems"
  ...
  >
    <template #header-actions>
      <el-button round @click="router.push({ name: 'province-dashboard' })">返回总览</el-button>
    </template>

    <template #stats>
      <DashboardMetricCard eyebrow="待审备案" :value="pendingFilingCount" accent="blue" description="当前待审核企业备案数量" />
      <DashboardMetricCard eyebrow="待终审月报" :value="pendingReportCount" accent="green" description="等待省级终审的月报数量" />
      <DashboardMetricCard eyebrow="系统账户" :value="users.length" accent="orange" description="当前系统用户总数" />
      <DashboardMetricCard eyebrow="交换日志" :value="exchangeLogs.length" accent="red" description="最近数据交换记录数量" />
    </template>
      ...
      <div class="workspace-grid">
        <article class="surface-card">
          <div class="surface-card__header">
            <div>
              <h3>待处理事项</h3>
              <p>优先处理备案审核和待省级终审月报。</p>
            </div>
          </div>
          <div class="surface-card__body" style="display:grid;gap:12px">
            <el-button type="primary" round @click="activeNav = 'filing'">进入备案审核</el-button>
            <el-button type="success" round @click="activeNav = 'reports'">进入报表终审</el-button>
            <el-button round @click="activeNav = 'users'">进入用户管理</el-button>
            <el-button round @click="activeNav = 'system'">进入系统维护</el-button>
          </div>
        </article>
        ...
        <article class="surface-card">
          <div class="surface-card__header">
            ...
            <el-button link type="primary" @click="loadMonitor">刷新</el-button>
          </div>
      ...
      <div class="workspace-grid">
        <article class="surface-card">
          <div class="surface-card__header">
            ...
            <el-button link type="primary" @click="activeNav = 'filing'">查看全部</el-button>
          </div>
          <div class="surface-card__body">
            <el-table :data="pendingEnterprisesPreview" stripe>
              ...
              <el-table-column prop="filing_status" label="状态" min-width="120">
                <template #default="{ row }">{{ filingStatusText(row.filing_status) }}</template>
              </el-table-column>
            </el-table>
          </div>
        </article>
        ...
        <article class="surface-card">
          <div class="surface-card__header">
            ...
            <el-button link type="primary" @click="activeNav = 'reports'">查看全部</el-button>
          </div>
          <div class="surface-card__body">
            <el-table :data="pendingReportsPreview" stripe>
      ...
      <div class="surface-card__body">
        <el-form :inline="true" :model="enterpriseQuery" style="margin-bottom:12px">
          ...
          <el-form-item label="备案状态">
            <el-select v-model="enterpriseQuery.filing_status" clearable style="width:180px">
        ...
        <el-table :data="enterprises" stripe>
          ...
          <el-table-column prop="organization_code" label="组织机构代码" min-width="150" />
          <el-table-column prop="name" label="企业名称" min-width="180" />
          <el-table-column prop="industry" label="所属行业" min-width="160" />
          <el-table-column prop="filing_status" label="状态" min-width="120">
            <template #default="{ row }">{{ filingStatusText(row.filing_status) }}</template>
          </el-table-column>
          <el-table-column label="操作" min-width="260">
            <template #default="{ row }">
              <el-space wrap>
                <el-button type="primary" size="small" @click="viewEnterprise(row)">查看</el-button>
                <el-button type="success" size="small" @click="auditFiling(row.id, 'APPROVE')">通过</el-button>
                <el-button type="danger" size="small" @click="openFilingReject(row.id)">退回</el-button>
              </el-space>
            </template>
          </el-table-column>
        </el-table>
    ...
    <article v-else-if="activeNav === 'reports'" class="surface-card">
      <div class="surface-card__header">
        <div>
          <h3>报表管理</h3>
          <p>支持终审归档、退回修改、数据修订、删除和导出。</p>
        </div>
      </div>
      <div class="surface-card__body">
        <el-form :inline="true" :model="reportQuery" style="margin-bottom:12px">
          ...
          <el-form-item label="审核状态">
            <el-select v-model="reportQuery.review_status" clearable style="width:180px">
              <el-option label="待市级审核" value="PENDING_CITY_REVIEW" />
              <el-option label="待省级审核" value="PENDING_PROVINCE_REVIEW" />
              <el-option label="已归档" value="ARCHIVED" />
              <el-option label="已上报部级" value="REPORTED_TO_MINISTRY" />
              <el-option label="已退回" value="REJECTED" />
            </el-select>
          </el-form-item>
    ...
    <article v-else-if="activeNav === 'users'" class="surface-card">
      <div class="surface-card__header">
        <div>
          <h3>用户管理</h3>
          <p>支持按角色、地区、企业属性和统计周期筛选系统用户。</p>
        </div>
      </div>
      <div class="surface-card__body">
        <el-form :inline="true" :model="userQuery" style="margin-bottom:12px">
          ...
          <el-form-item label="角色">
            <el-select v-model="userQuery.role" clearable style="width:120px">
          ...
          <el-form-item label="企业名称"><el-input v-model="userQuery.enterprise_name" clearable /></el-form-item>
          <el-form-item label="备案状态">
            <el-select v-model="userQuery.filing_status" clearable style="width:180px">
              <el-option label="待审核" value="PENDING" />
              <el-option label="已通过" value="APPROVED" />
              <el-option label="已退回" value="REJECTED" />
            </el-select>
          ...
          <el-form-item label="启用状态">
            <el-select v-model="userQuery.is_active" clearable style="width:140px">
          ...
          <el-form-item><el-button @click="resetUserQuery">清空</el-button></el-form-item>
    ...
    <article v-else-if="activeNav === 'roles'" class="surface-card">
      <div class="surface-card__header">
        <div>
          <h3>角色管理</h3>
          <p>维护自定义角色及其权限集合。</p>
        </div>
        <el-button type="success" round @click="openRoleEditor()">新增角色</el-button>
      </div>
      <div class="surface-card__body">
        <el-table :data="roles" stripe>
          <el-table-column prop="name" label="角色名称" min-width="160" />
          <el-table-column prop="scope_role" label="适用范围" min-width="120">
            <template #default="{ row }">{{ roleText(row.scope_role) }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" min-width="180">
            <template #default="{ row }">
              <el-space>
                <el-button type="primary" link @click="openRoleEditor(row)">编辑</el-button>
      ...
      <div class="surface-card__body">
        ...
        <el-table :data="notifications" stripe>
          ...
          <el-table-column label="操作" min-width="180">
            <template #default="{ row }">
              <el-space>
                <el-button type="primary" link @click="openNotificationEditor(row)">编辑</el-button>
    ...
    <article v-else-if="activeNav === 'system'" class="surface-card">
      <div class="surface-card__header">
        <div>
          <h3>系统维护</h3>
          <p>配置上报时间窗口并查看实时系统监控。</p>
        </div>
      </div>
      <div class="surface-card__body">
        <el-form :inline="true" :model="windowForm" style="margin-bottom:16px">
        ...
        <el-button round @click="loadMonitor">刷新监控</el-button>
      </div>
    </article>
    ...
    <el-dialog v-model="filingRejectVisible" title="备案退回说明" width="520px">
      <el-input v-model="filingRejectRemark" type="textarea" :rows="4" maxlength="500" />
      <template #footer>
        <el-button @click="filingRejectVisible = false">取消</el-button>
        <el-button type="primary" @click="confirmFilingReject">确认退回</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="reportRejectVisible" title="省级退回修改说明" width="520px">
      ...
      <template #footer>
        <el-button @click="reportRejectVisible = false">取消</el-button>
        <el-button type="primary" @click="confirmReportReject">确认退回</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="revisionVisible" title="报表修订" width="520px">
      ...
      <el-select v-model="revisionForm.reduction_type" placeholder="减少类型" style="width:100%;margin-bottom:12px">
        <el-option v-for="item in reductionTypes" :key="item" :label="item" :value="item" />
      </el-select>
      <el-select v-model="revisionForm.primary_reason" placeholder="主要原因" style="width:100%;margin-bottom:12px">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="saveRevision">保存修订</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="roleEditorVisible" :title="editingRoleId ? '编辑角色' : '新增角色'" width="640px">
      <el-form :model="roleForm" label-width="100px">
        ...
        <el-form-item label="适用范围">
          <el-select v-model="roleForm.scope_role" style="width:100%">
            <el-option label="省级" value="PROVINCE" />
            <el-option label="市级" value="CITY" />
            <el-option label="企业" value="ENTERPRISE" />
          </el-select>
        ...
        <el-form-item label="权限">
          <el-select v-model="roleForm.permission_codes" multiple collapse-tags style="width:100%">
            <el-option v-for="item in permissions" :key="item.code" :label="item.code" :value="item.code" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="notificationEditorVisible" :title="editingNotificationId ? '编辑通知' : '新增通知'" width="620px">
      ...
      <template #footer>
        <el-button @click="notificationEditorVisible = false">取消</el-button>
    ...
    <el-drawer v-model="enterpriseDetailVisible" title="企业备案详情" size="520px">
      <el-descriptions v-if="selectedEnterprise" :column="1" border>
        ...
        <el-descriptions-item label="主营业务">{{ selectedEnterprise.main_business }}</el-descriptions-item>
    ...
    <el-drawer v-model="reportDetailVisible" title="月报详情" size="520px">
      <el-descriptions v-if="selectedReport" :column="1" border>
        ...
        <el-descriptions-item label="状态">{{ reviewStatusText(selectedReport.review_status) }}</el-descriptions-item>
...
const currentUserName = localStorage.getItem('username') ?? '省级管理员'
...
const navItems = computed(() => [
  ...
  { key: 'filing', label: '备案审核', badge: pendingFilingCount.value || undefined },
  { key: 'reports', label: '报表管理', badge: pendingReportCount.value || undefined },
  { key: 'users', label: '用户管理' },
  { key: 'roles', label: '角色管理' },
  { key: 'notifications', label: '通知管理', badge: notifications.value.length || undefined },
  { key: 'system', label: '系统维护' },
  { key: 'exchange', label: '数据交换', badge: exchangeLogs.value.length || undefined },
])

const pendingFilingCount = computed(() => enterprises.value.filter((item) => item.filing_status === 'PENDING').length)
const pendingReportCount = computed(() => reports.value.filter((item) => item.review_status === 'PENDING_PROVINCE_REVIEW').length)
const pendingEnterprisesPreview = computed(() => enterprises.value.filter((item) => item.filing_status === 'PENDING').slice(0, 5))
const pendingReportsPreview = computed(() => reports.value.filter((item) => item.review_status === 'PENDING_PROVINCE_REVIEW').slice(0, 5))
...
const loadEnterprises = async () => {
  const { data } = await http.get('/api/enterprises', {
    params: {
      filing_status: enterpriseQuery.filing_status || undefined,
      region: enterpriseQuery.region || undefined,
      name: enterpriseQuery.name || undefined,
    },
  })
...
const exportEnterprises = async () => {
  const response = await http.get('/api/province/enterprises/export', {
    params: {
      filing_status: enterpriseQuery.filing_status || undefined,
      region: enterpriseQuery.region || undefined,
      name: enterpriseQuery.name || undefined,
  ...
  downloadBlob(response.data, 'enterprise_filings.xlsx')
}

const resetEnterpriseQuery = async () => {
  enterpriseQuery.region = ''
  enterpriseQuery.name = ''
  enterpriseQuery.filing_status = 'PENDING'
  await loadEnterprises()
}

const loadReports = async () => {
  const { data } = await http.get('/api/employment-reports', {
    params: {
      report_month: reportQuery.report_month || undefined,
      review_status: reportQuery.review_status || undefined,
    },
  })
  reports.value = data
}
...
const loadUsers = async () => {
  const { data } = await http.get('/api/users', {
    params: {
      ...
      region: userQuery.region || undefined,
      enterprise_name: userQuery.enterprise_name || undefined,
      filing_status: userQuery.filing_status || undefined,
      nature: userQuery.nature || undefined,
      industry: userQuery.industry || undefined,
      is_active: userQuery.is_active,
      report_month: userQuery.report_month || undefined,
      report_quarter: userQuery.report_quarter || undefined,
      start_date: userQuery.start_date || undefined,
      end_date: userQuery.end_date || undefined,
    },
  })
  users.value = data
}
...
const resetUserQuery = async () => {
  ...
  userQuery.role = ''
  userQuery.region = ''
  userQuery.enterprise_name = ''
  userQuery.filing_status = ''
  userQuery.nature = ''
  userQuery.industry = ''
...
const openFilingReject = (id: number) => {
  filingRejectId.value = id
  filingRejectRemark.value = ''
  filingRejectVisible.value = true
}

const confirmFilingReject = async () => {
  if (!filingRejectId.value || !filingRejectRemark.value) return
  await auditFiling(filingRejectId.value, 'REJECT', filingRejectRemark.value)
  filingRejectVisible.value = false
}

const viewEnterprise = (row: any) => {
  selectedEnterprise.value = row
  enterpriseDetailVisible.value = true
}

const finalAudit = async (id: number) => {
  try {
    await http.post(`/api/employment-reports/${id}/final-audit`)
...
const openReportReject = (id: number) => {
  reportRejectId.value = id
  reportRejectRemark.value = ''
  reportRejectVisible.value = true
}
...
const reportToMinistry = async (id: number) => {
  try {
    await http.post(`/api/employment-reports/${id}/report-to-ministry`)
    ElMessage.success('已上报部级')
    await loadReports()
  } catch (error: any) {
...
const deleteReport = async (id: number) => {
  try {
    const result = await ElMessageBox.prompt('请输入删除原因', '删除报表')
    if (!result.value) return
    await http.delete(`/api/employment-reports/${id}`, { params: { remark: result.value } })
...
const openRevision = (row: any) => {
  revisionReportId.value = row.id
  revisionForm.note = ''
  revisionForm.baseline_employees = row.baseline_employees
  revisionForm.current_employees = row.current_employees
  revisionForm.reduction_type = row.reduction_type || ''
  revisionForm.primary_reason = row.primary_reason || ''
  revisionForm.primary_reason_detail = row.primary_reason_detail || ''
  revisionVisible.value = true
}

const saveRevision = async () => {
  if (!revisionReportId.value) return
  try {
    await http.post(`/api/employment-reports/${revisionReportId.value}/revisions`, revisionForm)
...
const viewRevisions = async (id: number) => {
  const { data } = await http.get(`/api/employment-reports/${id}/revisions`)
  revisionHistory.value = data
  revisionListVisible.value = true
}

const openUserEditor = (row?: any) => {
  editingUserId.value = row?.id ?? null
  userForm.username = row?.username ?? ''
  userForm.password = ''
  userForm.role = row?.role ?? 'CITY'
  userForm.region = row?.region ?? 'Kunming'
  userForm.is_active = row?.is_active ?? true
...
const saveUser = async () => {
  try {
    if (editingUserId.value) {
      await http.put(`/api/users/${editingUserId.value}`, { ...userForm, password: userForm.password || undefined })
    } else {
      await http.post('/api/users', userForm)
    }
    ElMessage.success('用户保存成功')
    userEditorVisible.value = false
    await loadUsers()
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.detail ?? '用户保存失败')
  }
}
...
const exportUsers = async () => {
  ...
  downloadBlob(response.data, 'users.xlsx')
}

const openRoleEditor = (row?: any) => {
  editingRoleId.value = row?.id ?? null
  roleForm.name = row?.name ?? ''
  roleForm.scope_role = row?.scope_role ?? 'CITY'
  roleForm.permission_codes = row?.permissions?.map((item: any) => item.code) ?? []
...
const saveRole = async () => {
  try {
    if (editingRoleId.value) {
      await http.put(`/api/roles/${editingRoleId.value}`, roleForm)
    } else {
      await http.post('/api/roles', roleForm)
    }
    ElMessage.success('角色保存成功')
    roleEditorVisible.value = false
    await loadRoles()
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.detail ?? '角色保存失败')
  }
}
...
const openNotificationEditor = (row?: any) => {
  editingNotificationId.value = row?.id ?? null
  notificationForm.title = row?.title ?? ''
  notificationForm.content = row?.content ?? ''
  notificationEditorVisible.value = true
}

const saveNotification = async () => {
  try {
    if (editingNotificationId.value) {
      await http.put(`/api/notifications/${editingNotificationId.value}`, notificationForm)
    } else {
      await http.post('/api/notifications', notificationForm)
    }
    ElMessage.success('通知保存成功')
    notificationEditorVisible.value = false
    await loadNotifications()
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.detail ?? '通知保存失败')
  }
}
...
const saveWindow = async () => {
  try {
    await http.post('/api/system/reporting-windows', windowForm)
...
const exportReports = async () => {
  const response = await http.get('/api/province/reports/export', {
    params: { report_month: reportQuery.report_month || undefined },
    responseType: 'blob',
  })
  downloadBlob(response.data, 'employment_reports.xlsx')
}

const exportToMinistry = async () => {
  try {
    const { data } = await http.get('/api/integration/ministry/export', { params: { report_month: exchangeForm.report_month } })
    ElMessage.success(`已导出并上报 ${data.row_count} 条数据`)
    await Promise.all([loadExchangeLogs(), loadReports()])
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.detail ?? '交换失败')
  }
}
